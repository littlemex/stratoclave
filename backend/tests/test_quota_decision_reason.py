"""F2 (CONTRACT-F2-grant.md): R26 — a decision carries a reason, returned to
the requester.

design-F2.md Ambiguity #5: `decision_comment` doubles as the returned
...
def _seed(tenant, request_id, requested_amount):
    ...
    seed_request(
        ...
        requested_amount_microusd=requested_amount, requested_expires_at=9_999_999_999,
    )
...
def test_r26_approve_for_full_amount_does_not_require_a_comment(dynamodb_mock, quota_events_table):
    from mvp import grants
...
def test_r26_approve_for_less_requires_a_comment(dynamodb_mock, quota_events_table):
    from mvp import grants
...
def test_r26_approve_for_less_stores_and_returns_the_comment(dynamodb_mock, quota_events_table):
    from mvp import grants
    ...
    assert result["decision_comment"] == "partial only, revisit next month"
