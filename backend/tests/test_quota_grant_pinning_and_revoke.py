"""F2 (CONTRACT-F2-grant.md): R10 + R23.
...
def test_r10_grant_approved_in_p_and_revoked_in_p_plus_1_moves_only_ps_row(
    ...
    # Revoke reads target_pk/target_sk/period straight off the grant row —
    # NOT from `current_period()` — so it moves P's row even though "now" is
    # well into P+1.
    txn_items = repo.revoke_grant_txn_items(
        tenant_id=TENANT, grant_id="g-pinned", approved_amount_microusd=400,
        target_pk=TENANT, target_sk=budget_sk(period_p), to_status="EXPIRED",
        revoked_by="sweeper", revoked_at="2026-09-15T00:00:00+00:00",
    )
...
def test_r23_early_revoke_moves_the_three_attributes_once(dynamodb_mock, quota_events_table):
    ...
    result = grants.revoke_grant(actor=actor, tenant_id=TENANT, grant_id="g-early", reason="no longer needed")
...
def test_r23_later_sweep_over_an_early_revoked_grant_is_a_no_op(dynamodb_mock, quota_events_table):
    ...
    grants.revoke_grant(actor=actor, tenant_id=TENANT, grant_id="g-early2", reason="done")
