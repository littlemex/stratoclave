"""F2 (CONTRACT-F2-grant.md): R6 + R31 — the two security-critical ids.
...
design-F2.md's exact expressions (R31 row):
...
def _build_full_transaction(*, actor_user_id: str, actor_is_admin: bool, tenant_id: str,
    ...
    condition_check = _authority_condition_check_item(
        actor_user_id=actor_user_id, actor_is_admin=actor_is_admin, tenant_id=tenant_id,
    )
    grant_put = repo.grant_put_txn_item(
        tenant_id=tenant_id, grant_id=grant_id, request_id=request_id,
        approver_user_id=actor_user_id, approved_amount_microusd=amount,
...
def test_r31_condition_check_is_transaction_item_zero_and_shaped_per_design(
    ...
    design-F2.md exactly for both actor kinds."""
    _seed_two_tenants()
    from mvp.grants import _authority_condition_check_item

    admin_check = _authority_condition_check_item(
        actor_user_id="admin-1", actor_is_admin=True, tenant_id="tenant-a")
    assert "ConditionCheck" in admin_check
    assert admin_check["ConditionCheck"]["Key"] == {"tenant_id": "tenant-a"}
    assert admin_check["ConditionCheck"]["ConditionExpression"] == "attribute_exists(tenant_id)"

    tl_check = _authority_condition_check_item(
        actor_user_id="tl-a", actor_is_admin=False, tenant_id="tenant-a")
    assert tl_check["ConditionCheck"]["Key"] == {"tenant_id": "tenant-a"}
    assert tl_check["ConditionCheck"]["ConditionExpression"] == "team_lead_user_id = :a"
    assert tl_check["ConditionCheck"]["ExpressionAttributeValues"] == {":a": "tl-a"}
...
def test_r31_team_lead_who_owns_tenant_a_is_refused_on_a_request_for_tenant_b(
    ...
    txn = _build_full_transaction(
        actor_user_id="tl-a", actor_is_admin=False, tenant_id="tenant-b",
...
def test_r31_admin_may_approve_any_existing_tenant(dynamodb_mock, quota_events_table):
    ...
    txn = _build_full_transaction(
        actor_user_id="admin-1", actor_is_admin=True, tenant_id="tenant-b",
...
def test_r6_authority_is_read_live_by_the_transaction_not_cached_by_approve(
    ...
    grant1 = grants.approve_limit_raise(
        ...
        expires_at=2_000_000_000, now_epoch=1_800_000_000,
    )
    assert grant1["status"] == "ACTIVE"
    ...
    with pytest.raises(grants.AuthorityDenied):
        grants.approve_limit_raise(
            ...
            expires_at=2_000_000_000, now_epoch=1_800_000_100,
        )
...
def test_r31_approve_limit_raise_refuses_team_lead_on_unowned_tenant(
    ...
    with pytest.raises(grants.AuthorityDenied):
        grants.approve_limit_raise(
            ...
            approved_amount_microusd=1_000, expires_at=2_000_000_000,
