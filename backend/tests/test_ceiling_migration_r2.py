...
rate sentinel (`__system__`/`SEAT_RATE`), never a live read of
...
def _seed_stored_rate(usd: int) -> None:
    """Simulate R20's stored-rate sentinel already being seeded by M1 --
    `dynamo.tenant_budgets.assert_seat_rate_or_seed` (R20) does not exist
    yet, so this is a raw write standing in for "M1 has run on this
    environment". This file's evidence is about M2 reading THIS value, not
    about how it got seeded (that mechanism is R20's own test file)."""
    TenantBudgetsRepository()._table.put_item(Item={
        "tenant_id": "__system__",
        "sk": "SEAT_RATE",
        "seat_monthly_usd": Decimal(usd),
    })
...
def test_per_seat_row_with_a_non_integer_quotient_is_adjudicated_not_migrated(dynamodb_mock):
    ...
    assert summary.get("adjudicated", 0) >= 1
    adjudicated_ids = {
        entry.get("tenant_id") for entry in summary.get("adjudication_list", [])
    }
...
def test_backfill_is_idempotent(dynamodb_mock):
    ...
    assert second["migrated"] == 0, "a second pass over an already-migrated row must be a no-op"
    row = _raw(tenant_id, period)
    assert int(row["seat_count"]) == 2
