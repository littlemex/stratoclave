...
# --------------------------------------------------------------------------- #
# the OpenAI-compatible endpoint transport
# --------------------------------------------------------------------------- #


class TestopenaiTokenReuse:
    def test_token_is_minted_once_per_ttl(self, monkeypatch):
        from mvp import _openai_transport

        _openai_transport.reset_transport_cache_for_tests()
        calls = []
        monkeypatch.setattr(
            _openai_transport, "mint_bearer_token", lambda region: calls.append(region) or "tok"
        )

        first = _openai_transport.auth_headers("us-east-2")
        second = _openai_transport.auth_headers("us-east-2")
    ...
    def test_expired_token_is_re_minted(self, monkeypatch):
        ...
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", lambda region: next(minted))

        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer first"
        # Expire it by making the clock jump past the TTL.
        base = _openai_transport._now()
        monkeypatch.setattr(
            _openai_transport,
            "_now",
            lambda: base + _openai_transport.DEFAULT_TOKEN_TTL.total_seconds() + 1,
        )
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer second"
    ...
    def test_refresh_happens_before_expiry(self):
        ...
        assert _openai_transport._TOKEN_REFRESH_MARGIN > timedelta(0)
        assert _openai_transport._TOKEN_REFRESH_MARGIN < _openai_transport.DEFAULT_TOKEN_TTL
    ...
    def test_expiry_is_re_checked_after_waiting_for_the_lock(self, monkeypatch):
        ...
        _openai_transport.reset_transport_cache_for_tests()
        clock = {"now": 1600.0}
        monkeypatch.setattr(_openai_transport, "_now", lambda: clock["now"])
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", lambda region: "fresh")

        # An entry that is already expired at 1600, so the waiter takes the lock.
        _openai_transport._tokens["us-east-2"] = ("old", 1500.0)
        lock = _openai_transport._token_lock("us-east-2")
        lock.acquire()

        result = {}
        waiter = threading.Thread(
            target=lambda: result.update(_openai_transport.auth_headers("us-east-2"))
        )
        waiter.start()
        ...
        _openai_transport._tokens["us-east-2"] = ("minted-while-waiting", 1900.0)
        clock["now"] = 2000.0
        lock.release()
    ...
    def test_regions_do_not_serialise_on_one_lock(self):
        ...
        _openai_transport.reset_transport_cache_for_tests()
        a = _openai_transport._token_lock("us-east-2")
        b = _openai_transport._token_lock("us-west-2")
        assert a is not b
        assert _openai_transport._token_lock("us-east-2") is a
    ...
    def test_tokens_are_per_region(self, monkeypatch):
        ...
        _openai_transport.reset_transport_cache_for_tests()
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", lambda region: f"tok-{region}")
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer tok-us-east-2"
        assert _openai_transport.auth_headers("us-west-2")["Authorization"] == "Bearer tok-us-west-2"


class TestopenaiTokenInvalidation:
    def test_an_upstream_rejection_drops_the_cached_bearer(self, monkeypatch):
        ...
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer dead"
        _openai_transport.invalidate_token("us-east-2")
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer live"
    ...
    def test_a_stale_rejection_does_not_discard_a_fresh_token(self, monkeypatch):
        ...
        _openai_transport.reset_transport_cache_for_tests()
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", lambda region: "fresh")
        _openai_transport.auth_headers("us-east-2")

        stale = {"Authorization": "Bearer dead"}
        _openai_transport.invalidate_token("us-east-2", stale)
        assert _openai_transport._tokens.get("us-east-2", ("",))[0] == "fresh"

        # The token that WAS rejected still goes.
        _openai_transport.invalidate_token("us-east-2", {"Authorization": "Bearer fresh"})
        assert "us-east-2" not in _openai_transport._tokens
    ...
    def test_invalidation_does_not_wait_on_the_region_lock(self, monkeypatch):
        ...
        _openai_transport.reset_transport_cache_for_tests()
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", lambda region: "tok")
        _openai_transport.auth_headers("us-east-2")
    ...
    def test_invalidating_one_region_leaves_the_others(self, monkeypatch):
        ...
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", mint)
        _openai_transport.auth_headers("us-east-2")
        _openai_transport.auth_headers("us-west-2")
        _openai_transport.invalidate_token("us-east-2")
        _openai_transport.auth_headers("us-east-2")
        _openai_transport.auth_headers("us-west-2")
...
class TestopenaiTokenRefreshWindow:
    def test_inside_the_margin_the_refresh_leaves_the_calling_thread(self, monkeypatch):
        ...
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer tok1"
        assert threads == [caller], "a cold mint is the caller's own work"

        ttl = _openai_transport.DEFAULT_TOKEN_TTL.total_seconds()
        margin = _openai_transport._TOKEN_REFRESH_MARGIN.total_seconds()
        clock["now"] += ttl - margin + 1

        served = _openai_transport.auth_headers("us-east-2")["Authorization"]
        assert served == "Bearer tok1", "the caller must be served the cached token"
        released.set()
        _openai_transport._drain_refresher()
        ...
        assert _openai_transport._tokens["us-east-2"][0] == "tok2"
    ...
    def test_the_async_path_refreshes_inside_the_margin_too(self, monkeypatch):
        ...
        served = asyncio.run(_openai_transport.auth_headers_async("us-east-2"))
    ...
    def test_a_refresh_failure_keeps_serving_the_valid_token(self, monkeypatch):
        ...
        clock = {"now": 1000.0}
        monkeypatch.setattr(_openai_transport, "_now", lambda: clock["now"])
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", lambda region: "tok1")
        _openai_transport.auth_headers("us-east-2")

        def boom(region):
            raise RuntimeError("mint is down")

        monkeypatch.setattr(_openai_transport, "mint_bearer_token", boom)
        ttl = _openai_transport.DEFAULT_TOKEN_TTL.total_seconds()
        margin = _openai_transport._TOKEN_REFRESH_MARGIN.total_seconds()
        clock["now"] += ttl - margin + 1
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer tok1"
    ...
    def test_past_expiry_the_caller_does_wait(self, monkeypatch):
        ...
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer tok1"
        clock["now"] += _openai_transport.DEFAULT_TOKEN_TTL.total_seconds() + 1
        assert _openai_transport.auth_headers("us-east-2")["Authorization"] == "Bearer tok2"


class TestopenaiAsyncAuth:
    def test_a_cache_hit_answers_without_a_thread(self, monkeypatch):
        from mvp import _openai_transport

        _openai_transport.reset_transport_cache_for_tests()
        monkeypatch.setattr(_openai_transport, "mint_bearer_token", lambda region: "tok")
        _openai_transport.auth_headers("us-east-2")  # warm
        ...
        got = asyncio.run(_openai_transport.auth_headers_async("us-east-2"))
    ...
    def test_a_cold_mint_leaves_the_event_loop(self, monkeypatch):
        ...
        async def run():
            ...
            headers = await _openai_transport.auth_headers_async("us-east-2")
...
class TestopenaiClientPooling:
    def test_sync_client_is_pooled_per_region(self):
        from mvp import _openai_transport

        _openai_transport.reset_transport_cache_for_tests()
        first = _openai_transport.sync_client("us-east-2")
        assert _openai_transport.sync_client("us-east-2") is first
        assert _openai_transport.sync_client("us-west-2") is not first

    def test_async_client_is_pooled_per_region(self):
        from mvp import _openai_transport

        _openai_transport.reset_transport_cache_for_tests()
        first = _openai_transport.async_client("us-east-2")
        assert _openai_transport.async_client("us-east-2") is first
        assert _openai_transport.async_client("us-west-2") is not first
    ...
    def test_pooled_client_carries_no_authorization(self):
        ...
        _openai_transport.reset_transport_cache_for_tests()
        client = _openai_transport.sync_client("us-east-2")
    ...
    def test_there_is_no_per_call_override(self):
        ...
        for factory in (_openai_transport.sync_client, _openai_transport.async_client):
    ...
    def test_connection_ceiling_is_configurable(self, monkeypatch):
        ...
        _openai_transport.reset_transport_cache_for_tests()
        monkeypatch.setenv(_openai_transport._MAX_CONNECTIONS_ENV, "512")
        assert _openai_transport._limits().max_connections == 512
        # Keepalive must match, or a connection closed between requests puts the
        # handshake back on the hot path.
        assert _openai_transport._limits().max_keepalive_connections == 512
    ...
    def test_an_unusable_ceiling_is_rejected(self, monkeypatch, bad):
        ...
        monkeypatch.setenv(_openai_transport._MAX_CONNECTIONS_ENV, bad)
        with pytest.raises(CapacityConfigError):
            _openai_transport.openai_transport_connection_ceiling()

    def test_an_unset_ceiling_uses_the_default(self, monkeypatch):
        from mvp import _openai_transport

        monkeypatch.delenv(_openai_transport._MAX_CONNECTIONS_ENV, raising=False)
        assert (
            _openai_transport.openai_transport_connection_ceiling()
            == _openai_transport._DEFAULT_MAX_CONNECTIONS
        )
    ...
    def test_idle_connections_outlive_a_gap_between_bursts(self):
        ...
        limits = _openai_transport._limits()
    ...
    def test_pool_wait_is_bounded(self):
        ...
        pool_timeout = _openai_transport._DEFAULT_TIMEOUT.pool
    ...
    def test_the_nonstreaming_deadline_sits_below_the_cdn_timeout(self):
        ...
        assert _openai_transport._DEFAULT_TIMEOUT.read > CDN_ORIGIN_TIMEOUT_SECONDS
    ...
    def test_a_closed_client_is_rebuilt(self):
        ...
        first = _openai_transport.sync_client("us-east-2")
        first.close()
        second = _openai_transport.sync_client("us-east-2")
