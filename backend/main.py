...
from mvp.chat_completions import router as mvp_chat_completions_router
...
#
# The DynamoDB-backed limiter (core/rate_limit_ddb.py) raises
# HTTPException(429) directly from each route's decorator, so FastAPI's
# built-in exception handling emits the 429 — no middleware or app.state
# wiring is needed. Decorators live inside each router (cognito_auth,
# sso_exchange, ...). Per-IP counters are shared across all ECS tasks.
# ---------------------------------------------------------------------------
...
app.include_router(mvp_chat_completions_router)  # POST /v1/chat/completions
