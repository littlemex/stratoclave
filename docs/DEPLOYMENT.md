...
**Money-affecting flags are the four at the top of that group** (`STRATOCLAVE_UNOBSERVED_HOLDS`, `STRATOCLAVE_HARD_CEILING_GATE`, `STRATOCLAVE_MEASURE_UNENFORCED_BOUND`, `STRATOCLAVE_POOL_HOLD_TTL_SECONDS`). Each is off or derived by default, and each has a design document that states what turning it on assumes.
