...
| **C14.33** (PR 3) A fourth admission limit, `user_dollar_quota`, bounds what ONE member of a tenant may spend in a period — in the tenant's money unit, across every model at once, not tokens. It contributes an item to the SAME atomic admission transaction as the other three walls, is declared `grantable=False` (money-denominated does not make a limit raisable — C14.22's own rule, extended to a fourth wall), and its own public blocker name (`personal_spend`) is distinct from the token wall's (`personal_budget`), so a client cannot mistake one refusal for the other. | B | `mvp/routing/user_dollar_quota.py:build_reserve_txn_items`, declared in `mvp/reserve_limits.py:RESERVE_LIMITS` and mapped in `mvp/grants.py:_BLOCKER_BY_WALL` |
| **C14.34** (PR 3) The base this ceiling admits against for a period is fixed once, by the FIRST admission that needs it, from a period-keyed default on the tenant row — never recomputed per host or per request, so no two hosts need to agree on the wall clock to enforce the same figure. Sealing is a compare-and-set on the version the setter last bumped, not merely on the seal's own absence: a setter write landing between the sealer's read and its write is caught by the version clause, closing the interleaving where an `attribute_not_exists`-only condition would let the sealer commit a value the setter had already superseded. A sealed period's base cannot change, by construction of that same condition; the mid-period lever for an individual member is a raise, not shipped for this wall (C14.24's own reasoning, restated for a fourth wall), never an edit to the sealed figure. An empty default history reads as "unconfigured", never as an error — the wall simply contributes no item, rather than wedging every admission for a tenant that has not opted in. | **E** | `test_user_dollar_quota_tenant_defaults.py` — in particular the deterministic interleaving where a setter's write lands between the sealer's read and its write, which asserts the FINAL sealed value is the setter's rather than merely that a retry happened: a seal conditioned only on the seal's own absence passes the weaker check and fails this one. Enforced by `dynamo/tenants.py:TenantsRepository.seal_user_dollar_base` (the version-CAS'd conditional write) and `.set_user_dollar_default` (the setter's own `attribute_not_exists(sealed_user_dollar_base.#p)` guard) |
| **C14.35** (PR 3) This wall's reservation is given back on all three terminal paths — a reaper reclaim, a settle, a release — and each give-back is composed into the SAME `TransactWriteItems` as the per-model quota's own give-back on that path, never a second bare `UpdateItem`: a crash between two independent writes is exactly the estimate-ceiling drift this rule exists to prevent. Every reversal is keyed by the period THIS reservation actually reserved against (frozen on the hold, for the reclaim path; carried on the reservation context, for settle/release), never a fresh current-period computation, for the same midnight-crossing reason the per-model quota already observes elsewhere in this section. **Boundary: an adjustment arriving after the row's TTL is lost, in either direction** — the same boundary the per-model quota accepts, extended here rather than tightened. | B | `mvp/routing/user_dollar_quota.py:build_reverse_txn_item` / `build_adjust_txn_item`, composed by `mvp/_pipeline.py`'s hold give-back and `_release_or_settle_quota_and_uq` |
...
**C14.33 and C14.35 hold at B for the identical reason**, under the same split authorship
for this feature. **C14.34 is E, and the difference is the mechanism rather than the
provenance of its test.** Everything that clause asserts is enforced by a DynamoDB
conditional write on one item: a sealed period's base cannot change because the setter's
condition refuses it, and a setter that returns success has genuinely changed the base
because the sealer's version clause refuses to commit over a history that moved. Nothing
there is best-effort and nothing depends on two hosts agreeing on a clock. Recording it at
B because its tests came from a blind split would be a statement about who wrote the test,
which is not what a level means — and understating a level in the document a reader
consults to find where the guarantees stop is the defect PR 2' existed to correct, in the
...
**Closed by PR 3 (C14.33-C14.35).** A per-user ceiling denominated in money existed as
an open item here; `user_dollar_quota` is that reserve, settle and release path, so the
gap this paragraph named no longer applies. It is not restated under Open items below —
a closed clause leaves the list rather than being re-described as done.
