...
| **C14.32** NOT GUARANTEED. Nothing checks that a scheduled job's stack passes the tables its handler reads. Every repository resolves its table as `os.getenv("DYNAMODB_<X>_TABLE", "stratoclave-<x>")`, so an unset variable does not fail loudly — it names a DIFFERENT deployment's table, and the only thing standing between that and a cross-deployment read is an IAM refusal. The service task never reaches the fallback because its stack passes every table; a scheduled job reaches it whenever its stack passes only the tables somebody remembered. | N | Not implemented. Two instances are known, one fixed and one not, and both are named under Open items; closing the class means one check comparing the repositories a handler constructs against the environment its stack gives it |
