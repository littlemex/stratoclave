...
| **C14.23** A figure that EQUALS the ceiling currently in force is refused while any of that ceiling is granted, and a tenant is not retired while any grant still bears capacity. The first: an operator means "make the baseline this", the setter would read it as "and the grant still sits on top", so the ceiling jumps by the granted amount and at expiry drops by it twice, landing below the number typed. The second: archiving over a live grant leaves it pinned to a row nobody will look at again, counted against a cap for a tenant that no longer exists, with no path that can ever release it. | B | `mvp/admin_tenants.py:apply_pool_budget_request` — shared by the admin and team-lead routes so neither can drift — and `archive_tenant`, which drains through `mvp/grants.py:revoke_all_active_grants` (grant-first) before deleting and refuses on any remainder |
...
- **C14.30, the routing cascade never tries a cheaper candidate once a pricier one hits
  ...
  on a path this project treats as its highest-risk one — recorded in `SEAMS.md` S11 as
  its own future change rather than folded into F3's hint work, which only names the
  candidates the code actually priced.
