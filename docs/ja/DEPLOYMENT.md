...
- AWS CDK v2 を対象アカウントとリージョンで**最初の `cdk deploy` 前にブートストラップ**しておくこと。`cdk synth` による構成確認だけなら bootstrap は不要である。
...
| Python            | 3.11+           | バックエンドのテスト実行およびローカルリビルドに必要。最近の macOS 同梱 Python は 3.9 系のため、`brew install python@3.12` または `pyenv install 3.11.9` などで新しい版を入れること。 |
| Rust              | 1.75+           | `stratoclave` CLI をリビルドする場合のみ必要。Apple Silicon では cold な `cargo build --release` は約 2 分かかり、およそ 500 crate をコンパイルする — ハングではない。 |
| Docker            | 最近のバージョン | ヘルパースクリプトはバックエンドイメージのビルドに `docker` コマンドを呼び出す。Docker Desktop は macOS / Windows / Linux で動作する。 |
...
`deploy-all.sh` が成功すると、依存関係順に **9 つの CloudFormation スタック**がプロビジョニングされる。スタック名の接頭辞は `STRATOCLAVE_PREFIX` (デフォルト: `stratoclave`) で制御される。以下の `<Prefix>` は PascalCase 形式である。

| # | スタック                 | リソース                                                             | 目的 |
| - | --------------------- | --------------------------------------------------------------------- | ------- |
| 1 | `<Prefix>NetworkStack`  | VPC, パブリックサブネット 2 つ, SG, VPC Flow Logs (CloudWatch, 30 日)          | パブリックサブネットのみのネットワーク (NAT なし)。ALB SG のインバウンドは AWS 管理の **CloudFront origin-facing prefix list** に限定されている — ALB DNS への直接プローブは L4 で失敗する。 |
| 2 | `<Prefix>DynamodbStack` | 13 個の DynamoDB テーブル。`users`, `user-tenants`, `tenants`, `usage-logs` (RETAIN), `api-keys` (RETAIN + PITR), **`sso-nonces`** (TTL、vouch-by-STS のリプレイ防御用) を含む | すべての永続状態、`PAY_PER_REQUEST`。監査上重要なテーブルは `cdk destroy` 後も残存する。 |
| 3 | `<Prefix>EcrStack`      | プライベート ECR リポジトリ (RETAIN)                                       | バックエンドのコンテナイメージを保持する。最終手段のロールバック面。 |
| 4 | `<Prefix>AlbStack`      | インターネット向け ALB, ターゲットグループ, :80 の HTTP リスナー, 本番では `deletionProtection=true` | バックエンドのパブリックエントリポイント。リスナーは `open:false` を使うので、ALB SG に 0.0.0.0/0 のイングレスは穴開けされない。 |
| 5 | `<Prefix>WafStack`      | **WAFv2 WebACL** (CLOUDFRONT スコープ、`us-east-1`): CommonRuleSet, KnownBadInputs, IpReputation, rate-based (5 分ウィンドウ、IP 単位), 任意の IPSet 許可リスト | `ENABLE_WAF=false` でオプトアウト可能。ARN は FrontendStack からクロススタック参照される。 |
| 6 | `<Prefix>FrontendStack` | S3 バケット (プライベート、SSL 強制), CloudFront ディストリビューション (**OAC**, minTLS 1.2_2021, HSTS 730 日 + 厳格な CSP + `frame-ancestors 'none'` を持つ ResponseHeadersPolicy), SPA フォールバック関数 | 静的 Web UI ホスティング。レガシーな OAI ではなく **Origin Access Control** を使用 — S3 バケットポリシーは `aws:SourceArn` でスコープされる。 |
| 7 | `<Prefix>CognitoStack`  | User Pool, app client, hosted-UI ドメイン                               | 認証。CloudFront ドメインをインポートしてコールバック URL を配線する。 |
| 8 | `<Prefix>EcsStack`      | ECS クラスター, Fargate サービス (1 タスク), タスクロール, タスク定義     | バックエンドランタイム。すべての環境変数がここに注入される。 |
| 9 | `<Prefix>ConfigStack`   | SSM Parameter Store のエントリ                                           | バックエンドとヘルパースクリプトが消費する静的ランタイム値。 |

アーカイブされたスタック (RDS, Redis, CodeBuild, Verified Permissions) は `iac/lib/_archived/` にあり、デフォルトでは**デプロイされない**。参考として残されている。以前の `WafStack` のアーカイブは、上記の CLOUDFRONT スコープで稼働する WAF に置き換えられた。

### Synth 時のセキュリティチェック (cdk-nag)

すべての `cdk synth` / `cdk deploy` は `cdk-nag` の `AwsSolutionsChecks` Aspect を実行する。既知かつドキュメント化されたトレードオフ (CloudFront のデフォルト証明書、managed prefix list によるイングレス、ENFORCED ではない Cognito advanced security など) は、理由コメント付きで `iac/bin/iac.ts` にて抑制されている。cdk-nag の**新しい** `[Error at ...]` は `cdk synth` を失敗させる。`CDK_NAG=off` はローカルでのごく稀なデバッグセッションでのみ設定すること。
...
| WAFv2 (5 ルール + WebACL アソシエーション)          | ~$10 ~ $12 (固定) |
| VPC Flow Logs → CloudWatch (30 日保持)              | ~$1 |
| DynamoDB (PAY_PER_REQUEST, 小規模な書き込み量)    | ~$1 |
| S3 + CloudWatch Logs + SSM Parameter Store        | ~$1 |
| **小計**                                      | **~$42 ~ $45** |

使い捨てやサンドボックス用途のスタックでは `ENABLE_WAF=false` を設定して WAF の費用 (~$10) を除外できる。
...
`iac/scripts/` 内の以下のスクリプトは過去のイテレーションの残り物であり、デフォルトフローの一部ではない。将来のリリースで削除される予定である。新しいドキュメントやランブックに追加しないこと。

- `validate-config.sh`
- `deploy-with-update.sh`

通常のデプロイでは、インフラ + フロントエンドに `iac/scripts/deploy-all.sh`、バックエンドイメージに `iac/scripts/build-and-push.sh` を使うこと。`iac/scripts/cloud-build.sh` は、ローカルの Docker ソケットが使えない環境向けの任意のリモートビルドドライバである。明示的に必要でなければ無視しても安全である。
...
# requirements-dev.txt は pytest, moto, joserfc, ruff も取り込むため、同じ
# venv でテストスイートを実行できる。本番 Docker イメージは
# requirements.txt のみをインストールする。
...
| `CORS_ORIGINS`                           | はい (`localhost` 不可)    | はい         | 本番では CloudFront ドメインと一致する必要がある。開発/ローカルのバックエンドは `http://localhost:3003` を設定できる。 |
| `DYNAMODB_SSO_NONCES_TABLE`              | 任意                | はい (デフォルト `stratoclave-sso-nonces`) | Vouch-by-STS のリプレイ防御 (`backend/dynamo/sso_nonces.py`)。テーブルが欠けている場合、バックエンドは警告ログを出し、±5 分のスキューチェックのみにフォールバックする。 |
| `ENABLE_WAF`                             | IaC のみ           | n/a         | CDK 側のフラグ (`iac/bin/iac.ts` で読み取られる)。`false` にすると `<Prefix>WafStack` のプロビジョニングをスキップする。デフォルト: `true`。 |
| `WAF_RATE_LIMIT_PER_5MIN`                | IaC のみ           | n/a         | rate-based ルールにおける IP 単位のリクエスト上限。デフォルト: `300`。 |
| `WAF_IP_ALLOWLIST_ENABLED`               | IaC のみ           | n/a         | SSM パラメータで裏打ちされた IPSet 許可リストを有効化する。デフォルト: `false`。 |
| `CDK_NAG`                                | IaC のみ           | n/a         | `off` にすると synth 時の cdk-nag aspect をスキップする。デフォルト: `on`。 |
