...
- 管理者から渡された Stratoclave のデプロイメント URL。たとえば `https://d8b03j8erit4k.cloudfront.net` (本ガイドでは具体例として用いるが、実際には自分のデプロイメント URL に置き換えること)。
- 以下のいずれかのサインイン方式:
...
```bash
stratoclave setup https://d8b03j8erit4k.cloudfront.net   # あなたのデプロイメント URL
```
